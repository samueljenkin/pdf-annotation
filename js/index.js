...
//   var pdfData = atob($('#pdfBase64').val());
  ...
  function getNextPage () {
	if(currentPageNum < totalPages && pdfObj) {
		getPage(pdfObj, currentPageNum + 1)
	}
  }

  function getPrevPage () {
	if(currentPageNum > 1 && pdfObj) {
		getPage(pdfObj, currentPageNum - 1)
	}
  }
  ...
  const handleOpenPdfFile = () => {
	const fileInputRef = document.getElementById("pdfFileInput")
    if (fileInputRef) {
      fileInputRef.click();
    }
  };